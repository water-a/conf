...
test('.reset()', t => {
    t.context.configWithSchema = new source_1.default({
        cwd: tempy.directory(),
        schema: {
            foo: {
                default: 42
    ...
    t.context.configWithDefaults = new source_1.default({
        cwd: tempy.directory(),
        defaults: {
            foo: 42,
            bar: 99
...
test('encryption', t => {
    const config = new source_1.default({ cwd: tempy.directory(), encryptionKey: 'abc123' });
    t.is(config.get('foo'), undefined);
    t.is(config.get('foo', '🐴'), '🐴');
    config.set('foo', fixture);
...
test('encryption - corrupt file', t => {
    const cwd = tempy.directory();
    const before = new source_1.default({ cwd, encryptionKey: 'abc123' });
    before.set('foo', fixture);
    t.is(before.get('foo'), fixture);
